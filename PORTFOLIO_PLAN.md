...
- [x] 4. **Content Collections**: used **`src/content.config.ts`** (Astro's current root-level convention — `src/content/config.ts` is now the legacy path) with the `file()` loader from `astro/loaders` pointing at plain JSON files in `src/content/*.json`, rather than one Markdown/frontmatter file per entry — a closer match for structured list data. All four collections (`projects`, `experience`, `skills`, `certifications`) built with the exact schemas from this plan and seeded with the real CV facts listed above. `npx astro check` passes with 0 errors/0 warnings (some pre-existing Zod-v4-internal deprecation _hints_ on the classic `z.object()` chain API, harmless and out of scope here). The Endava underwriting-workbench project card is still deliberately **not** added, per the Follow-ups NDA-confirmation item below.
...
## Phase D — Tooling & Quality Gates — ✅ done (2026-09-15)

- [x] ESLint (`eslint` + `eslint-plugin-astro` + TS parser) and Prettier (+ `prettier-plugin-astro`) so `npm run lint` / `npm run format` actually do something meaningful — this is what the `PostToolUse` hook (already configured, see Tooling & Automation above) calls after every component edit. (Done in Phase A.)
- [x] Husky + lint-staged pre-commit hook running lint+format on staged files — a second, git-level backstop that also covers commits made outside Claude Code (manual edits, other tools), on top of the editor-level hook. `npx husky init` wired `core.hooksPath` to `.husky/_`; `.husky/pre-commit` runs `npx lint-staged`; config lives in `package.json`'s `lint-staged` key (`eslint --fix` then `prettier --write` on staged files, scoped by extension).
- [x] `astro check` for TypeScript/template validation, run in CI. Added a `check` npm script (`astro check`) and a `npm run check` step in `deploy.yml` between lint and build, now that `@astrojs/check`/`typescript` are installed (pulled forward during Phase C).
